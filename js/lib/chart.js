...
const ChartView = widgets.DOMWidgetView.extend({
    render: function() {
        ...
        if (check_callback_tooltips_beforetitle){
            options.tooltips.callbacks.beforeTitle = eval(options.tooltips.callbacks.beforeTitle);}
        if (check_callback_tooltips_title){
            options.tooltips.callbacks.title = eval(options.tooltips.callbacks.title);}
        if (check_callback_tooltips_aftertitle){
            options.tooltips.callbacks.afterTitle = eval(options.tooltips.callbacks.afterTitle);}
        if (check_callback_tooltips_beforebody){
            options.tooltips.callbacks.beforeBody = eval(options.tooltips.callbacks.beforeBody);}
        if (check_callback_tooltips_beforelabel){
            options.tooltips.callbacks.beforeLabel = eval(options.tooltips.callbacks.beforeLabel);}
        if (check_callback_tooltips_label){
            options.tooltips.callbacks.label = eval(options.tooltips.callbacks.label);}
        if (check_callback_tooltips_labelcolor){
            options.tooltips.callbacks.labelColor = eval(options.tooltips.callbacks.labelColor);}
        if (check_callback_tooltips_labeltextcolor){
            options.tooltips.callbacks.labelTextColor = eval(options.tooltips.callbacks.labelTextColor);}
        if (check_callback_tooltips_afterlabel){
            options.tooltips.callbacks.afterLabel = eval(options.tooltips.callbacks.afterLabel);}
        if (check_callback_tooltips_afterbody){
            options.tooltips.callbacks.afterBody = eval(options.tooltips.callbacks.afterBody);}
        if (check_callback_tooltips_beforefooter){
            options.tooltips.callbacks.beforeFooter = eval(options.tooltips.callbacks.beforeFooter);}
        if (check_callback_tooltips_footer){
            options.tooltips.callbacks.footer = eval(options.tooltips.callbacks.footer);}
        if (check_callback_tooltips_afterfooter){
            options.tooltips.callbacks.afterFooter = eval(options.tooltips.callbacks.afterFooter);}
        if (check_callback_tooltips_custom){
            options.tooltips.callbacks.custom = eval(options.tooltips.callbacks.custom);}


        if (check_callback_legend){
            options.legendCallback = eval(options.legendCallback);}
        if (check_callback_legend_onclick){
            options.legend.onClick = eval(options.legend.onClick);}


        if (check_callback_animation_progress){
            options.animation.onProgress = eval(options.animation.onProgress);}
        if (check_callback_animation_complete){
            options.animation.onComplete = eval(options.animation.onComplete);}


        if (check_callback_xticks){
            options.scales.xAxes.ticks.callback = eval(options.scales.xAxes.ticks.callback);}
        if (check_callback_yticks){
            options.scales.yAxes.ticks.callback = eval(options.scales.yAxes.ticks.callback);}
        if (check_callback_xticks_major){
            options.scales.xAxes.ticks.major.callback = eval(options.scales.xAxes.ticks.major.callback);}
        if (check_callback_yticks_major){
            options.scales.yAxes.ticks.major.callback = eval(options.scales.yAxes.ticks.major.callback);}
        if (check_callback_xticks_beforeupdate){
            options.scales.xAxes.beforeUpdate = eval(options.scales.xAxes.beforeUpdate);}
        if (check_callback_yticks_beforeupdate){
            options.scales.yAxes.beforeUpdate = eval(options.scales.yAxes.beforeUpdate);}
        if (check_callback_xticks_beforesetdimensions){
            options.scales.xAxes.beforeSetDimensions = eval(options.scales.xAxes.beforeSetDimensions);}
        if (check_callback_yticks_beforesetdimensions){
            options.scales.yAxes.beforeSetDimensions = eval(options.scales.yAxes.beforeSetDimensions);}
        if (check_callback_xticks_aftersetdimensions){
            options.scales.xAxes.afterSetDimensions = eval(options.scales.xAxes.afterSetDimensions);}
        if (check_callback_yticks_aftersetdimensions){
            options.scales.yAxes.afterSetDimensions = eval(options.scales.yAxes.afterSetDimensions);}
        if (check_callback_xticks_beforedatalimits){
            options.scales.xAxes.beforeDataLimits = eval(options.scales.xAxes.beforeDataLimits);}
        if (check_callback_yticks_beforedatalimits){
            options.scales.yAxes.beforeDataLimits = eval(options.scales.yAxes.beforeDataLimits);}
        if (check_callback_xticks_afterdatalimits){
            options.scales.xAxes.afterDataLimits = eval(options.scales.xAxes.afterDataLimits);}
        if (check_callback_yticks_afterdatalimits){
            options.scales.yAxes.afterDataLimits = eval(options.scales.yAxes.afterDataLimits);}
        if (check_callback_xticks_beforebuildticks){
            options.scales.xAxes.beforeBuildTicks = eval(options.scales.xAxes.beforeBuildTicks);}
        if (check_callback_yticks_beforebuildticks){
            options.scales.yAxes.beforeBuildTicks = eval(options.scales.yAxes.beforeBuildTicks);}
        if (check_callback_xticks_afterbuildticks){
            options.scales.xAxes.afterBuildTicks = eval(options.scales.xAxes.afterBuildTicks);}
        if (check_callback_yticks_afterbuildticks){
            options.scales.yAxes.afterBuildTicks = eval(options.scales.yAxes.afterBuildTicks);}
        if (check_callback_xticks_beforeticktolabelconversion){
            options.scales.xAxes.beforeTickToLabelConversion = eval(options.scales.xAxes.beforeTickToLabelConversion);}
        if (check_callback_yticks_beforeticktolabelconversion){
            options.scales.yAxes.beforeTickToLabelConversion = eval(options.scales.yAxes.beforeTickToLabelConversion);}
        if (check_callback_xticks_afterticktolabelconversion){
            options.scales.xAxes.afterTickToLabelConversion = eval(options.scales.xAxes.afterTickToLabelConversion);}
        if (check_callback_yticks_afterticktolabelconversion){
            options.scales.yAxes.afterTickToLabelConversion = eval(options.scales.yAxes.afterTickToLabelConversion);}
        if (check_callback_xticks_beforecalculatetickrotation){
            options.scales.xAxes.beforeCalculateTickRotation = eval(options.scales.xAxes.beforeCalculateTickRotation);}
        if (check_callback_yticks_beforecalculatetickrotation){
            options.scales.yAxes.beforeCalculateTickRotation = eval(options.scales.yAxes.beforeCalculateTickRotation);}
        if (check_callback_xticks_aftercalculatetickrotation){
            options.scales.xAxes.afterCalculateTickRotation = eval(options.scales.xAxes.afterCalculateTickRotation);}
        if (check_callback_yticks_aftercalculatetickrotation){
            options.scales.yAxes.afterCalculateTickRotation = eval(options.scales.yAxes.afterCalculateTickRotation);}
        if (check_callback_xticks_beforefit){
            options.scales.xAxes.beforeFit = eval(options.scales.xAxes.beforeFit);}
        if (check_callback_yticks_beforefit){
            options.scales.yAxes.beforeFit = eval(options.scales.yAxes.beforeFit);}
        if (check_callback_xticks_afterfit){
            options.scales.xAxes.afterFit = eval(options.scales.xAxes.afterFit);}
        if (check_callback_yticks_afterfit){
            options.scales.yAxes.afterFit = eval(options.scales.yAxes.afterFit);}
        if (check_callback_xticks_afterupdate){
            options.scales.xAxes.afterUpdate = eval(options.scales.xAxes.afterUpdate);}
        if (check_callback_yticks_afterupdate){
            options.scales.yAxes.afterUpdate = eval(options.scales.yAxes.afterUpdate);}

        
        if (check_callback_pointlabels){
            options.pointLabels.callback = eval(options.pointLabels.callback);}
